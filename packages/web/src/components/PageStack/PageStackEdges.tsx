...
/** Subtle sliver of the Pages stacked above the one currently in view — a few thin
 *  bars peeking in from the top edge of the scrollable Page region (App.tsx),
 *  narrower and fainter the further back in the stack they represent, so there's a
 *  constant at-a-glance sense of "how deep is this stack" without a number or any
 *  real layout height. Purely decorative (aria-hidden) — the PageNav dot row is the
 *  actual interactive/accessible way to see and jump between Pages. No bottom
 *  equivalent: that edge sits right above the dot row, which already communicates
 *  position in the stack on its own. */
export function PageStackEdges({ above }: PageStackEdgesProps) {
