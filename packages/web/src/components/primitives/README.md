...
| `Overlay` | A centered modal (backdrop + card-like box) | Escape/outside-press both close it, via `useDismiss`. `className` extends `.overlay__box` for per-instance sizing/background (AnnotationDetail, SaveAsTemplateModal, TemplateBrowser). Doesn't portal itself — wrap the caller's own `createPortal` around it when needed. |
