...
Read: `wattle.this()`, `wattle.page.cards()`, `wattle.page.card(id)`, `wattle.card(id)`, `wattle.vault.search(query)` → `{cards, pages}`, `wattle.search(query, {mode:"vault"|"web"})`.
Draw: `wattle.ui.clear()`, `wattle.ui.html(html)`, `wattle.ui.text(str)`, `wattle.ui.field(name, {kind, label?, placeholder?, defaultValue?, options?, onSubmit?})` + `wattle.ui.value(name)` to read it back inside a handler (`onSubmit` runs on Enter, same rules as a button's `onClick`), `wattle.ui.button(label, onClick)`, `wattle.ui.list(items)` (strings or `{label, onClick}`), `wattle.ui.table(rows, columns?)`. `wattle.ui.clear()` wipes this card's WHOLE content — redraw everything (field, button, results) together in one `render()` function called after every change, not just the piece that changed, or controls like the button itself will vanish.
Mutate (handler only): `wattle.createCard({title, content, typeId?, pageId?})`, `wattle.editCard(id, {title?, content?})`, `wattle.removeCard(pageCardId)`, `wattle.generate(instructions, {context?:"page"|"own", present?:"card"|"here"})` (`"here"` returns `{html, title}` for you to draw with `wattle.ui.html`), `wattle.navigate.toPage(title)`.
