 ...
 *  with no rebuild or restart. Entirely static now for every mode — "interactive" used
 *  to have a dynamic `<!-- ACTIONS -->` splice (the self-running/AUTORUN action-card
 *  vocabulary), retired along with that mechanism (Brilliantly Simple Generation Agent
 *  plan) — real workspace mutation now goes through that plan's own tool-calling agent
 *  (agentCompiler.ts), not through a model-authored action-script card here. */
